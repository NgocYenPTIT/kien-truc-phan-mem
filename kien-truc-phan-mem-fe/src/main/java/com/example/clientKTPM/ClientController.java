...
public class ClientController {
    ...
    @Value("${app.global.url.tournament-invitation-service}")
    private String urlTournamentInvitationService;
...
//            System.out.println(session.getAttribute("user"));
    ...
    public String showDetailTournament(
        ...
        if (user != null) {
            ...
            if (tournament.getStatus().equals("KẾT THÚC")) {
                model.addAttribute("flag_join", "end");
                return "detail-tournament";
            }

            if (tournament.getOrganizer().trim().equals(user.getUsername().trim())) {// chủ giải đấu
                model.addAttribute("flag_join", "invite");
            } else {// không phải chủ giải đấu
                InvitationFlag flag = this.serviceAPI.call(
                        this.urlTournamentInvitationService + "check-flag?tournamentId=" + id + "&userId=" + user.getId(),
                        HttpMethod.GET,
                        null,
                        InvitationFlag.class,
                        (String) session.getAttribute("token")
                );
                model.addAttribute("flag_join", flag.getValue());
//                model.addAttribute("flag_join", "request");
//                model.addAttribute("flag_join", "requested");
//                model.addAttribute("flag_join", "out");
            }
...
//            System.out.println(model.getAttribute("tournament"));
    ...
    @PostMapping("/tournament/{id}/add-player")
    public String addPlayer(@PathVariable(name = "id") int id,
    ...
    @GetMapping("/tournament/{id}/invite-fail")
    public String inviteFailure(
            @PathVariable(name = "id") int id,
            @RequestParam(name = "playerName") String playerName,
            Model model) {
        User user = (User) session.getAttribute("user");
        ...
        if (user != null) {
            try {
                TournamentInvitation inv = this.serviceAPI.call(
                        this.urlTournamentInvitationService + "invitation",
                        HttpMethod.POST,
                        CreateInvitation.builder().tournamentId(tournamentId).userId(user.getId()).type("INVITED").status("PENDING").build(),
                        TournamentInvitation.class,
                        (String) session.getAttribute("token")
                );
    ...
    @PostMapping("/tournament/{id}/requested")
    public String requestedJoin(@PathVariable(name = "id") int id,
                              @RequestParam(name = "userId") Long userId,
                              @RequestParam(name = "tournamentId") Long tournamentId) {
