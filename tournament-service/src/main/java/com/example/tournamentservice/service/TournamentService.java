...
import com.example.tournamentservice.model.TournamentJoin;
...
public class TournamentService {
    ...
    public ResponseEntity<?> getDetail(Long id, HttpServletRequest request) {
        ...
        map.put("participants", participants.getTotalItems());
    ...
    public ResponseEntity<?> getList(String name, String flag, Long currentPage, Long pageSize, HttpServletRequest request) {
        ...
        switch (flag) {
            ...
            case "only-join":
                TournamentJoin tournamentJoin = this.serviceAPI.call(
                        this.urlTournamentInvitationService + "invitation-join?userId=" + playerId,
                        HttpMethod.GET,
                        null,
                        TournamentJoin.class,
                        (String) request.getAttribute("token")
                );
                System.out.println(tournamentJoin);
                if (hasName) {
                    tournamentPage = tournamentRepository.findByIdInAndNameContainingIgnoreCaseAndDeletedAtIsNull(tournamentJoin.getTournamentIds(),name, pageable);
                } else {
                    tournamentPage = tournamentRepository.findByIdInAndDeletedAtIsNull(tournamentJoin.getTournamentIds(), pageable);
                }
                break;
